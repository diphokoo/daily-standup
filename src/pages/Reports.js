...
function Reports() {
  const sprintStartDate = new Date('2026-02-02T00:00:00+02:00');
  ...
  const generateSprints = () => {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      if (today >= startDate && today <= endDate) currentSprintNumber = sprintNumber;

      if (sprintNumber >= currentSprintNumber - 1 && sprintNumber <= currentSprintNumber) {
        sprints.push({ number: sprintNumber, startDate, endDate, label: `Sprint ${sprintNumber}` });
      }
  ...
  const sprintDates = useMemo(() => {
    ...
    if (sprint) return { startDate: sprint.startDate.toISOString(), endDate: sprint.endDate.toISOString() };
    return null;
    ...
    <div className="p-4">
      <div className="d-flex justify-content-between align-items-center mb-3">
        ...
        <div className="d-flex align-items-center gap-2">
          <span className="text-muted">{sprintDateRange}</span>
          <select
